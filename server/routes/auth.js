...
router.get("/search", async (req, res) => {
    try {
        const { anonymousName } = req.query;

        if (!anonymousName) {
            return res.status(400).json({ message: "Anonymous name query required" });
        }

        const users = await User.find({
            anonymousName: { $regex: anonymousName, $options: "i" }
        }).select("_id email anonymousName");
